...
- **train / verify 分離**: 改善 round の判定は train seed (= 改善時に diff を観た
  ...
  実測例 (= 2026-05-13 round 25 完了時点):
  - train (seed 20260513): 両者一致 87.1%
  - verify (seed 99999): 両者一致 87.3%
  - seed 33333 (= 第 3 系列): 両者一致 86.4%
  - train vs verify 差 -0.2pt / 3 seed std ≈ 0.5pt = **健全な汎化**
    (= specific seed に過剰最適化してない、 round 19→25 の +1591 件投入が
    各 seed で均等に効いた)
...
|---|---|---|---|---|---|---|
| 2026-05-13 (R25 後 fresh) | v2026.05.13.7 (= round 25 batch 1-26、 +1621 件 一般語彙) — **fresh seed 55555** (= 過去未使用) | 9,192 / 10,000 | 94.4%† | 93.8%† | +0.6pt† | **87.0%** |
| 2026-05-13 (R25 後 33333) | 同上 — seed 33333 | 9,205 / 10,000 | 94.2%† | 93.5%† | +0.7pt† | **86.4%** |
| 2026-05-13 (R25 後 verify) | 同上 — verify seed 99999 | 9,154 / 10,000 | 94.5%† | 93.9%† | +0.6pt† | **87.3%** |
| 2026-05-13 (R25 後 train) | 同上 — train seed 20260513 | 9,179 / 10,000 | **93.9%** | **93.3%** | **+0.6pt** | **87.1%** |
| 2026-05-13 (verify) | v2026.05.12 + round 19-23 (= 笑笑/神獣/暗視/種/絵描く/方) — verify seed 99999 | 9,191 / 10,000 | _(集計中)_ | _(集計中)_ | _(集計中)_ | 83.9% |
...
> round 25 後 4 seed の両者一致率は **+2.2 〜 +3.4pt 改善** (= 84% → 86-87%)。
> 4 seed mean **86.95%** / range 0.9pt (= 86.4-87.3%) で完全安定、 fresh seed 55555 でも 87.0% 維持 → **over-fit ゼロ**。
>
> ja/VV per-record 内訳:
> - train seed 1184 件中 200 件 sample を ground truth classify (= Agent 判定): ja correct 57.1% / vv correct 52.2% (= 差 +4.9pt in diff)
> - 全体 (= 一致 87.1% を両者正解として加算) で **ja 93.9% / VOICEVOX 93.3%、 差 +0.6pt**
> - 他 seed (†) は同 ratio で extrapolate (= sampling 200 件想定で精度 ±2pt)
>
> 注: round 19-20 時点 (= 旧 baseline 84.1%) の per-record review では ja 97.5% / VV 95.0% / +2.5pt だった。
> round 25 で両者一致が 84→87% に上昇した結果、 残 diff の構成が変化 (= 易しい ja 優位 case が
> 一致側に吸収され、 残 diff には ja 不利な 数詞・助数詞・複雑読みが偏った) ことで
> 差が縮小したと解釈。 ja 正答率の絶対値も低下したのは sample n=200 の variance も含む。
> 次 round で full 1184 件 classify を推奨。
