...
- `schema_version`: **必須** (alpha.10〜、 lib が `[meta] schema_version = "2"`
  ...
  alpha era format dict は `tools/migrate_v2.py` で stamp 適用要。
...
migration tooling の履歴: `tools/migrate_v2_context.py` (= staging 機械変換) +
`tools/merge_migrated_context.py` (= 実 core/ への surgical merge) で 51 surface
を機械変換、 `docs/migration/v2_context/MIGRATION_REPORT.md` が当時の triage 記録。
