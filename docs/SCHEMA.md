...
#### ★ prev / next は **同じ文字種の連続** (重要)

matcher が見る 「token」 は Lindera の形態素ではなく、 **入力上で連続する同じ文字種の区間**
(lib の `prev_logical_token` / `next_logical_token`)。 このため:

| 入力 | 対象字 | prev に見える値 | next に見える値 |
|---|---|---|---|
| 話し合い中 | 中 | `い` (漢字の 合 で切れる) | — |
| 頭下げた | 頭 	| — | `下` (ひらがな げた で切れる) |
| 昨日今日声 | 声 | `昨日今日` (漢字連続全体) | — |

従って **漢字とかなをまたぐ値は永久に一致しない**。
`next_starts_any = ["下げ"]` や `prev_ends_any = ["奥の"]` は書いても動かないので、
先頭の同種部分だけを書く (`["下"]`) か、 entry として登録する (`"奥の間" = "オクノマ"`)。
`validate.py` の `check_run_scoped_values` が error にする (2026-09-19 に 26 件検出された)。

`prev_eq` / `next_eq` は **run 全体との一致**なので、 「昨日今日声」 のように
漢字が連続すると `prev_eq_any = ["今日"]` は外れる。 `prev_ends_any` にすれば拾えるが、
語が複合語の末尾に来る形 (一番街 / 一番組 / 大体外) を巻き込むので、 用途で選ぶこと。
...
#### 一般語と同形の姓: suffix match (`surnames.toml`)

「大谷 = オオヤ / オオタニ」 のように **姓が一般語と同形** の場合、 登録を控えるだけだと
「大谷さん」 が誤読のまま残る。 この場合は **default = 一般語読み + 敬称 suffix で姓読み**
という第 3 の形を採る (`tools/gen_surname_suffix.py` が生成):

```toml
"大谷" = { reading = "オオヤ", match = [ { next_starts_any = ["さん", "くん", "ちゃん", "君", "様", "氏", "先生", "選手", "監督", "議員"], reading = "オオタニ" } ] }
```

**注意 — entry を新設する副作用**: それまで dict に entry が無かった surface に entry を
足すと、 Lindera が人名として読めていた 「姓 + 名」 文脈まで一般語読みに固定される
(実測: 山中伸弥 = やまなか → さんちゅう、 平野歩夢 = ひらの → へいや)。
suffix 文脈だけ確認しても気付けないので、 **追加前後で 「姓 + 名」 を含む文脈を
batch-read して差分を取り、 suffix 以外が変わるなら追加しない**こと
(gen_surname_suffix.py の `--battery` と `EXCLUDED` はこのための仕組み)。

同じ形は地名にも使える (`国立 = コクリツ` だが `国立市 = クニタチ`、
suffix は 市 / 町 / 村 / 区 / 郡 / 駅 / 県 / 府)。
