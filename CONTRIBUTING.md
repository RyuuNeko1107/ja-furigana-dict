...
1. 該当ファイルを開く (ジャンル dir 階層):
   - 一般語 / 四字熟語 → [`core/jukugo/basic/`](core/jukugo/basic/)
   - 固有名詞 (会社・作品名・人名等) → [`core/jukugo/proper/`](core/jukugo/proper/)
   - 自然・生命系 (動植物・食べ物・地名・科学等) → [`core/jukugo/nature/`](core/jukugo/nature/)
   - 人文・芸術系 (古典文学・芸術・慣用句・宗教等) → [`core/jukugo/humanities/`](core/jukugo/humanities/)
   - 社会・制度系 (政治・金融・スポーツ等) → [`core/jukugo/society/`](core/jukugo/society/)
   - 物体・工芸系 (色・衣服・乗り物・建築等) → [`core/jukugo/objects/`](core/jukugo/objects/)
   - 単漢字フォールバック (水準別 5 ファイル) → [`core/unihan/`](core/unihan/)
     (常用 → `joyo.toml` / 人名用 → `jinmeiyou.toml` / 第1+第2水準 → `jis_basic.toml` /
     第3+第4水準 → `jis_supplement.toml` / 拡張 → `extension.toml`)
   - 単漢字 default override (issue #15 限定解) → [`core/single_overrides.toml`](core/single_overrides.toml)
   - 外来語 (英字始まり surface) → [`core/loanwords/it.toml`](core/loanwords/it.toml)
   - 作品造語 (作品単位 1 ファイル) → [`core/works/<medium>/<title>.toml`](core/works/)
   - 異体字 → [`core/compat.toml`](core/compat.toml)
   - 各 genre dir には [meta] description を持つ `_genre.toml` がある (sub-section heading)
...
### `core/unihan/*.toml` — 単漢字フォールバック (5 水準別)

漢字 1 文字 → カタカナ or ひらがな。 形態素解析でも辞書でもヒットしない単漢字の最終フォールバック。
**水準別に 5 ファイル** に分割されている (利用頻度の高い `joyo.toml` に review を集中させるため):

| ファイル | 内容 | 件数 (目安) |
|---|---|---:|
| `core/unihan/joyo.toml`           | 常用漢字 (文化庁告示 2,136 字)                  | ~2,136 |
| `core/unihan/jinmeiyou.toml`      | 人名用 (常用と重複しない 855 字)                  | ~853 |
| `core/unihan/jis_basic.toml`      | JIS 第1+第2水準 (CJK Basic Block の残り)         | ~13,000 |
| `core/unihan/jis_supplement.toml` | JIS 第3+第4水準 (Extension A + Compatibility)    | ~4,800 |
| `core/unihan/extension.toml`      | 拡張漢字 (Extension B 以降、 機械的扱い)          | ~22,800 |
